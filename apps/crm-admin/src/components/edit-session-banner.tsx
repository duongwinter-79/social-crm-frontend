 ...
 * Polls every 30 s — auto-syncs to R2 on the backend when Drive changes.
 * "Done editing" closes the session and deletes the Drive file.
 */
export function EditSessionBanner({ session, onExpired, onClosed }: Props) {
  ...
  // Notify parent when session expires server-side.
  useEffect(() => {
  ...
  function handleClose() {
    close.mutate(
      ...
      { onSuccess: onClosed, onError: onClosed },
    );
  }
  ...
  return (
    <div className="flex items-center gap-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-2.5 text-sm text-amber-900">
      ...
      >
        {close.isPending ? "Closing…" : "Done editing"}
      </button>
    </div>
  );
}
